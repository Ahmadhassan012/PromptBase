package com.promptbase.app.data.repository

import com.promptbase.app.data.db.PromptDao
...
@Singleton
class PromptRepository @Inject constructor(private val promptDao: PromptDao) {
    ...
    fun searchPromptsByTag(tagId: String, query: String): Flow<List<PromptWithTags>> {
        return promptDao.searchPromptsByTag(tagId, query)
    }

    fun getUntaggedPrompts(): Flow<List<PromptWithTags>> {
        return promptDao.getUntaggedPromptsWithTags()
    }

    fun searchUntaggedPrompts(query: String): Flow<List<PromptWithTags>> {
        return promptDao.searchUntaggedPromptsWithTags(query)
    }
    ...
    suspend fun importPrompts(imported: List<com.promptbase.app.util.ExportedPrompt>) {
